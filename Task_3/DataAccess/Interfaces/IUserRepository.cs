...
namespace DataAccess.Interfaces
{
    public interface IUserRepository : IRepositoryBase<User>
    {
        ...
        Task<IEnumerable<User>> GetUsersByRoleAsync(string role); // Получение пользователей по роли
        Task GrantAccessToCourseAsync(int userId, int courseId); // Предоставление доступа к курсу
        Task<IEnumerable<Course>> GetCoursesByUserAsync(int userId); // Получение курсов пользователя
